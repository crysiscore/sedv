...
BEGIN
UPDATE  mydb.stock SET  quantidade_recebida =quantidade_recebida+quant  WHERE  produto_Cod_Produto =codProduto;
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CadProduto`(IN Codigo int ,IN Nom varchar(50),In Preco Double,IN Unidad int,IN Categor int,IN Descric varchar(50), IN Foto blob)
BEGIN
...
BEGIN
SELECT distinct `Cod_Produto` AS Codigo, P.`nome`,`preco_unitario` AS Preco,`S.quantidade_recebida` as Quantidade, U.descricao_unidade AS Unidade, C.nome as Categoria
 FROM produto P, categoria C, unidade U, stock S
 WHERE `Cod_Produto`=codProduto and P.`unidade`=U.idUnidade and P.`categoria`=C.idCategoria and P.Cod_Produto=S.produto_Cod_Produto;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `EditarProduto`(IN Codigo int ,IN Nom varchar(50),In Preco Double,IN Unidad int,IN Categor int,IN Descric varchar(50), IN Foto blob)
BEGIN
...
BEGIN
 SELECT distinct  p.`Cod_Produto` AS Codigo,p.`nome`,`preco_unitario` AS Preco,`S.quantidade_recebida` as Quantidade,U.descricao_unidade as Unidade, C.nome as Categoria, p.`descricao`
 FROM produto p inner join unidade U on  p.Unidade=U.idUnidade 
inner join categoria C on  p.Categoria=C.idCategoria 
inner join stock S on p.Cod_Produto= S.Produto_Cod_Produto
...
BEGIN
 SELECT  `Cod_Produto` AS Codigo,P. `nome`,`preco_unitario` AS Preco,`S.quantidade_recebida` as Quantidade, U.descricao_unidade as Unidade, C.nome as Categoria, P.`descricao`
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateProduto`(IN codp int, IN nomep varchar(50), IN preco double, IN unidad int, IN cat int,IN descr varchar(200))
BEGIN
 UPDATE `mydb`.`produto` SET `nome` =nomep , `preco_unitario`=preco , `unidade` =unidad, `categoria` = cat , `descricao` =  descr 
...
-- Dump completed on 2022-01-31 20:54:37
